...
int readVariableLengthQuantity(char *inputPtr) {
	//Reads a varible length quantity from "inputPtr" and returns it as an integer
...
char readMTrkEvent(unsigned char **inputPP, struct mtrkEvent *outputPtr, char channelNumber) {
	//Reads a MIDI track event from the double pointer "inputPP"
	//Increases the pointer as the file is read through
	//Stores results in "outputPtr"
...
int combineMIDIFiles(char *outputBuffer, unsigned char *inputBuffer[], char inputBufferCount) {
	//Combines raw MIDI files passed with the "inputBuffer" array
	//Outputs the combined file to "outputBuffer"
